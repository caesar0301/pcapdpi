...
static void node_complete_timeout_flow_walker(const void *node, ndpi_VISIT which, int depth, void *user_data) {
    ...
    if ((which == preorder) || (which == leaf)){
        ...
        if ( timediff > 0 || flow->detection_completed ){
            ...
            if ( flow->detected_protocol == 0 && enable_protocol_guess) {
                flow->detected_protocol = ndpi_guess_undetected_protocol(ndpi_detection_module,
                                    flow->protocol,
                                    ntohl(flow->lower_ip),
                                    ntohs(flow->lower_port),
            ...
            if ( flow_hooker_root == NULL){
                ...
                flow_hooker_tail = flow_hooker_root;
            } else {
                flow_hooker_tail->next = flow;
                flow_hooker_tail = flow;
            }
            flow_hooker_tail->next = NULL;
        }
    }
}
...
static u_int32_t CLEAN_TICK = 3; // seconds
...
static void pcap_packet_callback(u_char * args, const struct pcap_pkthdr *header, const u_char * packet)
{
    ...
    detect_time = ((uint64_t) header->ts.tv_sec) * detection_tick_resolution +
            header->ts.tv_usec / (1000000 / detection_tick_resolution);
    if (lasttime > detect_time) {
        printf("\nWARNING: timestamp bug in the pcap file (ts delta: %lu, repairing)\n", lasttime - detect_time);
        detect_time = lasttime;
    }
    lasttime = detect_time;
    ...
    if (_pcap_datalink_type == DLT_EN10MB && type == htons(ETH_P_IP) && header->caplen >= sizeof(struct ndpi_ethhdr)) {
        ...
        v4_warning:
            if (ipv4_warning_used == 0) {
                printf("\nWRNING: only IPv4 packets are supported in this demo (nDPI supports both IPv4 and IPv6), all other packets will be discarded\n\n");
                ipv4_warning_used = 1;
            }
          return;
        }
        ...
        // process the packet
        packet_processing(detect_time, header, iph, header->len - ip_offset, header->len);

        // check if we clean timeout/completed flows
        time_t now;
        time(&now);
        if ( tree_clear_timestamp == 0 ){
            tree_clear_timestamp = (u_int32_t)now;
            return;
        }

        if ( difftime(now, tree_clear_timestamp ) >= CLEAN_TICK ){
            // walk through the tree to remove timeout flows
            ndpi_twalk(ndpi_flows_root, node_complete_timeout_flow_walker, &now);

            void *ret;
            int count = 0;
            struct ndpi_flow *ptr = flow_hooker_root, *tmp = NULL;
            while( ptr != flow_hooker_tail ){
